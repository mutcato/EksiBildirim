...
$(document).ready(function GetUserFromStorage(){
    chrome.storage.sync.get("id", function(data) {
        ...
        if(storageID==undefined){
            console.log("user will be defined...");
            //Create a user in z_chrome_users table and create a storage id in chrome
            
            ip2db(function(result){
                console.log("result : "+result);
            });
            console.log("2 storage id: " + storageID);
            //Check Google IDs if they exists insert it
        }
...
function getGoogleID(storageID){
    $.ajax({
        ...
        success: function(data){
            ...
            {
                postdata:{gmail:gmail,storageID:storageID}
            },
            function(data, status){
                console.log("data: " + data + "\nStatus: " + status);
            });
